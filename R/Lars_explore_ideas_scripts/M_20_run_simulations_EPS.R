  ...
  for (repetition_idx in seq_along(repetitions)) {
    ...
    for (n_combination_idx in seq_along(n_combinations_array)) {
      ...
      if (n_combination_idx %% floor((length(n_combinations_array)/10)) == 0) {
        saveRDS(MAE_dt, file.path(folder_save, paste0(file_name, "_MAE_repetition_", repetition, "_EPS_tmp.rds")))
      }
    ...
    saveRDS(MAE_dt_long, file.path(folder_save, paste0(file_name, "_MAE_repetition_", repetition, "_EPS.rds")))
    if (file.exists(file.path(folder_save, paste0(file_name, "_MAE_repetition_", repetition, "_EPS_tmp.rds")))) {
      file.remove(file.path(folder_save, paste0(file_name, "_MAE_repetition_", repetition, "_EPS_tmp.rds")))
    }
