...
for (rho in rhos) {
  ...
  for (repetition in seq(repetitions)) {
    ...
    for (strategy in new_strategies) {
      ...
      save_name = paste0("/mn/kadingir/biginsight_000000/lholsen/PhD/Paper3/Paper3_save_location/M_17_n_train_1000_n_test_500_rho_", rho, "_equi_FALSE_betas_2_10_0.25_-3_-1_1.5_-0.5_10_1.25_1.5_-2_3_-1_-5_4_-10_2_5_estimated_repetition_", repetition ,"_", strategy, ".rds")
      ...
      for (n_comb_now_idx in seq_along(n_combinations)) {
        ...
        if (n_combinations[n_comb_now_idx] == 2) {
          ...
          # Store the results
          save_list[[strategy]][[1]][[file_relevant_names[n_comb_now_idx]]] = list(shapley_values = dt_shapley,
                                                                                   MSEv = NULL,
